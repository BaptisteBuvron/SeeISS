...
class PredictTime
{
    ...
    public static function ThetaG($epoch, PredictDeepArg $deep_arg)
    {
        ...
        $GMST = PredictMath::Modulus($GMST + Predict::secday * Predict::omega_E * $UT, Predict::secday);
    ...
    public static function getCurrentDaynumFromUnix(int $time){
        return self::unix2daynum($time);
    }
    ...
    public static function daynum2readable($dn, $zone = 'America/Los_Angeles', $format = 'm-d-Y H:i:s')
    {
        ...
        if ($format == 'fr'){
            $date = new DateTimeFrench("@" . round($unix));
            $format = 'l j F Y';
        }
        else{
     ...
     *
     * @param PredictTLE $tle The TLE object
     *
     * @return int
     */
    public static function getEpochTimeStamp(PredictTLE $tle)
    {
        $year = $tle->epoch_year;
        $day  = $tle->epoch_day;

...
class Predict
{
     ...
     *       is GtkSatList).
     *
     * @param PredictSat $sat   The satellite data.
     * @param PredictQTH $qth   The observer data.
     * @param int         $maxdt The maximum number of days to look ahead.
     *
     * @return PredictPass Pointer instance or NULL if no pass can be
     *         found.
     */
    public function get_next_pass(PredictSat $sat, PredictQTH $qth, $maxdt)
    {
     ...
     */
    public function get_pass(PredictSat $sat_in, PredictQTH $qth, $start, $maxdt)
    {
        $aos = 0.0;    /* time of AOS */
        $tca = 0.0;    /* time of TCA */
        $los = 0.0;    /* time of LOS */
     ...
     *
     * @param PredictSat $sat     The satellite structure.
     * @param PredictQTH $qth     The QTH
     ...
     */
    public function get_sat_vis(PredictSat $sat, PredictQTH $qth, $jul_utc)
    {
        ...
        PredictSolar::Calculate_Solar_Position($jul_utc, $solar_vector);
        PredictSGPObs::Calculate_Obs($jul_utc, $solar_vector, $zero_vector, $obs_geodetic, $solar_set);

        if (PredictSolar::Sat_Eclipsed($sat->pos, $solar_vector, $eclipse_depth)) {
        ...
        if ($sat_sun_status) {
            $sun_el = PredictMath::Degrees($solar_set->el);
     ...
     *  @param PredictSat $sat   The satellite data.
     *  @param PredictQTH $qth   The observer's location (QTH) data.
     ...
     */
    public function find_aos(PredictSat $sat, PredictQTH $qth, $start, $maxdt)
    {
        $t = $start;
        $aostime = 0.0;
        ...
        */
        ...
        if (($sat->otype == PredictSGPSDP::ORBIT_TYPE_GEO) ||
            ($sat->otype == PredictSGPSDP::ORBIT_TYPE_DECAYED) ||
     ...
     *  @param PredictSat $sat The satellite data.
     *  @param PredictQTH $qth The QTH observer location data.
     *  @param float       $t   The time for calculation (Julian Date)
     *
     */
    public function predict_calc(PredictSat $sat, PredictQTH $qth, $t)
    {
        $obs_set      = new PredictObsSet();
        $sat_geodetic = new PredictGeodetic();
        $obs_geodetic = new PredictGeodetic();
        ...
        $sgpsdp = PredictSGPSDP::getInstance($sat);
        if ($sat->flags & PredictSGPSDP::DEEP_SPACE_EPHEM_FLAG) {
            $sgpsdp->SDP4($sat, $sat->tsince);
        } else {
            $sgpsdp->SGP4($sat, $sat->tsince);
        }

        PredictMath::Convert_Sat_State($sat->pos, $sat->vel);

        /* get the velocity of the satellite */
        $sat->vel->w = sqrt($sat->vel->x * $sat->vel->x + $sat->vel->y * $sat->vel->y + $sat->vel->z * $sat->vel->z);
        $sat->velo = $sat->vel->w;
        PredictSGPObs::Calculate_Obs($sat->jul_utc, $sat->pos, $sat->vel, $obs_geodetic, $obs_set);
        PredictSGPObs::Calculate_LatLonAlt($sat->jul_utc, $sat->pos, $sat_geodetic);
        ...
        $sat->ma *= 256.0 / 360.0;
        $sat->phase = PredictMath::Degrees($sat->phase);
     ...
     *  @author Alexandru Csete, OZ9AEC
     */
    public function find_los(PredictSat $sat, PredictQTH $qth, $start, $maxdt)
    {
        $t = $start;
        $lostime = 0.0;
     ...
     *  @param PredictSat $sat   The satellite to find AOS for.
     *  @param PredictQTH $qth   The ground station.
     ...
     * current pass.
     */
    public function find_prev_aos(PredictSat $sat, PredictQTH $qth, $start)
    {
        $aostime = $start;
     ...
     *  @author John A. Magliacane, KD2BD
     */
    public function has_aos(PredictSat $sat, PredictQTH $qth)
    {
         $retcode = false;
     ...
     */
    public function get_passes(PredictSat $sat, PredictQTH $qth, $start, $maxdt, $num = 0)
    {
        $passes = array();

...
require_once 'PredictSGSDPStatic.php';
require_once 'PredictSGPObs.php';
require_once 'PredictSolar.php';
require_once 'PredictDeepArg.php';
require_once 'PredictDeepStatic.php';
...
class PredictSat
{
    ...
    public function __construct(PredictTLE $tle)
    {
        $headerParts    = explode(' ', $tle->header);
        $this->name     = $headerParts[0];
        $this->nickname = $this->name;
        $this->tle      = $tle;
        $this->pos      = new PredictVector();
        $this->vel      = new PredictVector();
        $this->sgps     = new PredictSGSDPStatic();
        $this->deep_arg = new PredictDeepArg();
        $this->dps      = new PredictDeepStatic();
    ...
    public function select_ephemeris()
    {
        ...
        if (Predict::twopi / $xnodp / Predict::xmnpda >= .15625) {
            $this->flags |= PredictSGPSDP::DEEP_SPACE_EPHEM_FLAG;
        } else {
            $this->flags &= ~PredictSGPSDP::DEEP_SPACE_EPHEM_FLAG;
        }
    }
     ...
     */
    public function sat_data_init_sat(PredictSat $sat, PredictQTH $qth = null)
    {
        ...
        $jul_utc = PredictTime::Julian_Date_of_Epoch($sat->tle->epoch); // => tsince = 0.0
        ...
        $sdpsgp = PredictSGPSDP::getInstance($sat);
        ...
        $sat->velo = $sat->vel->w;
        PredictSGPObs::Calculate_Obs($jul_utc, $sat->pos, $sat->vel, $obs_geodetic, $obs_set);
        PredictSGPObs::Calculate_LatLonAlt($jul_utc, $sat->pos, $sat_geodetic);
    ...
    public function get_orbit_type(PredictSat $sat)
    {
         $orbit = PredictSGPSDP::ORBIT_TYPE_UNKNOWN;

         if ($this->geostationary($sat)) {
              $orbit = PredictSGPSDP::ORBIT_TYPE_GEO;
         } else if ($this->decayed($sat)) {
              $orbit = PredictSGPSDP::ORBIT_TYPE_DECAYED;
         } else {
              $orbit = PredictSGPSDP::ORBIT_TYPE_UNKNOWN;
         }
     ...
     */
    public function geostationary(PredictSat $sat)
    {
     ...
     *
     */
    public function decayed(PredictSat $sat)
    {
     ...
     */
    public function calculateApparentMagnitude($time, PredictQTH $qth)
    {
        ...
        $observerGeo      = new PredictGeodetic();
        ...
        $observerPos      = new PredictVector();
        $observerVel      = new PredictVector();
        $solarVector      = new PredictVector();
        PredictSolar::Calculate_Solar_Position($time, $solarVector);
        PredictSGPObs::Calculate_User_PosVel($time, $observerGeo, $observerPos, $observerVel);

        // Determine the solar phase and and thus the percent illumination
        $observerSatPos = new PredictVector();
        PredictMath::Vec_Sub($this->pos, $observerPos, $observerSatPos);

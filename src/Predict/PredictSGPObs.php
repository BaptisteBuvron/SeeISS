...
class PredictSGPObs
{
    ...
    public static function Calculate_User_PosVel(
        $_time, PredictGeodetic $geodetic, PredictVector $obs_pos, PredictVector $obs_vel
    )
    {
        ...
        $geodetic->theta = PredictMath::FMod2p(PredictTime::ThetaG_JD($_time) + $geodetic->lon);/*LMST*/
    ...
    public static function Calculate_LatLonAlt($_time, PredictVector $pos, PredictGeodetic $geodetic)
    {
        ...
        $geodetic->lon = PredictMath::FMod2p($geodetic->theta - PredictTime::ThetaG_JD($_time)); /*radians*/
        $r = sqrt(($pos->x * $pos->x) + ($pos->y * $pos->y));
        $e2 = Predict::__f * (2 - Predict::__f);
        $geodetic->lat = PredictMath::AcTan($pos->z, $r); /*radians*/

        do {
            $phi    = $geodetic->lat;
            $sinPhi = sin($phi);
            $c      = 1 / sqrt(1 - $e2 * ($sinPhi * $sinPhi));
            $geodetic->lat = PredictMath::AcTan($pos->z + Predict::xkmper * $c * $e2 * $sinPhi, $r);
    ...
    public static function Calculate_Obs($_time, PredictVector $pos, PredictVector $vel, PredictGeodetic $geodetic, PredictObsSet $obs_set)
    {
        $obs_pos = new PredictVector();
        $obs_vel = new PredictVector();
        $range   = new PredictVector();
        $rgvel   = new PredictVector();
